...
func (td TripleDipper) Cart(clt *http.Client) error {
	...
	if err != nil {
		return fmt.Errorf("posting cart request: %w", err)
	}
	defer resp.Body.Close()

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return fmt.Errorf("reading cart response body: %w", err)
	}
...
func (td TripleDipper) form(doc *html.Node) (url.Values, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("adding CSRF token to form: %w", err)
	}
	form.Add("_csrf", csrf)
	...
	for i, d := range td.Dippers {
		...
		if err != nil {
			return nil, fmt.Errorf("adding Item to form: %w", err)
		}
		...
		for _, e := range d.Extras {
			...
			if err != nil {
				return nil, fmt.Errorf("adding Extra to form: %w", err)
			}
